honest.Treebuild <- function(data, form.outcome, form.pscore, weights,  # subset, x, y,
  ...
  if (nrow(init.tree$frame) == 1) {
    ...
    # ans <- list(frame = tmp.frame,
    #             where = init.tree$where,
    #             call  = init.tree$call, 
    #             terms = init.tree$terms,
    #             cptable = init.tree$cptable,
    #             parms   = init.tree$parms,
    #             control = init.tree$control,
    #             splits  = init.tree$splits,
    #             num.leaf  = 1,
    #             leaf.size = nrow(data.train),
    #             leaf.obs  = 1:nrow(data.train))
    
    # class(ans) <- "causalPartitionTree"
  ...
  for (h in 1:nrow(init.tree$frame)) {
    ...
    var.left  <- 1/n.tar * sum((Y.star[ind.est.l] - trt.eff.l * v.hat[ind.est.l])^2
                                                    * v.hat[ind.est.l]^2) / (1/n.tar * sum(v.hat[ind.est.l] ^ 2))^2
    var.right <- 1/n.tar * sum((Y.star[ind.est.r] - trt.eff.r * v.hat[ind.est.r])^2
                                                    * v.hat[ind.est.r]^2) / (1/n.tar * sum(v.hat[ind.est.r] ^ 2))^2
  ...
  # confidence interval
  init.tree$frame$ci.lower <- est.trt.eff - sqrt(variance) * qnorm(0.95) # qnorm(1-(1-sqrt(0.9))/2) # 
  init.tree$frame$ci.upper <- est.trt.eff + sqrt(variance) * qnorm(0.95) # qnorm(1-(1-sqrt(0.9))/2) # 
  init.tree$frame$widthCI  <- init.tree$frame$ci.upper - init.tree$frame$ci.lower
  ...
  for (t in 1:length(subtree.seq)) { # t in 1:2
    ...
    if (nrow(tree.to.valid$frame) == 1) {
      ...
      for (h in 1:nrow(tree.to.valid$frame)) {
        ...
        var.left  <- 1/n.tar * sum((Y.star[ind.valid.l] - trt.eff.l * v.hat[ind.valid.l])^2
                                                 * v.hat[ind.valid.l]^2) / (1/n.tar * sum(v.hat[ind.valid.l]^2))^2
        var.right <- 1/n.tar * sum((Y.star[ind.valid.r] - trt.eff.r * v.hat[ind.valid.r])^2
                                                 * v.hat[ind.valid.r]^2) / (1/n.tar * sum(v.hat[ind.valid.r]^2))^2
        # Skip this loop if there is negative estimated variance
        # if (var.left < 0 | var.right < 0) {
        #   next
        # }
        
        valid.goodness <- valid.goodness + (delta.trt.eff) ^ 2  / (var.left/n.tar + var.right/n.tar)
