...
stemp <- function(y, wt, x, parms, continuous) {
  ...
  # x: vector of current split covariate values
  ...
  if (is.null(pscore) & is.null(cond.outcome)) {
    ...
    # data.node.update[] <- lapply(data.node.update,
...
etemp <- function(y, wt, parms) {
  ...
  # produce a deviance for the node along with a vector to be used as a label for the node.
  ...
  if (is.null(pscore) & is.null(cond.outcome)) {
    ...
    # tmp <- model.fit(data.node = data.node.update,
    #                  method = mthd, folds = 2,
  ...
  # variance <- sum((Y.star - trt.eff * v.hat)^2 * v.hat^2) / (sum(v.hat ^ 2))^2
  
  # list(label = trt.eff, deviance = variance)
  
  list(label = trt.eff, deviance = rss)
