Treebuild.GCTwithVar <- function(data, form.outcome, form.pscore, weights,  # subset, x, y,
  ...
  data.est   <- data.train
  est.modelFitBefore <- modelFitBefore
  ...
  if (modelFitBefore) {
    ...
    # cond.outcome  <- est.outcome(form.outcome, data.train, method = outcome.mthd, folds = 5) # cond.outcome.all[train.idx]
    # pscore        <- est.pscore(form.pscore, data.train, method = pscore.mthd, folds = 5) #pscore.all[train.idx]
    # 
  ...
  if (est.modelFitBefore) {
    ...
    data.est$cond.outcome <- cond.outcome
    data.est$pscore       <- pscore
